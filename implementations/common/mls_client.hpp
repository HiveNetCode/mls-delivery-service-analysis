...
{
public:
    MLSClient(typename DeliveryServiceI::NetworkType & network,
    ...
    void printTree()
    {
        ...
        for(uint32_t nodeIdx = 0; nodeIdx < tree.nodes.size(); ++nodeIdx)
        {
            const auto & node = tree.node_at(mls::NodeIndex{ nodeIdx });

            if(node.leaf())
            {
                const auto & leafNode = node.leaf_node();
                auto & identity = leafNode.credential.get<mls::BasicCredential>().identity;

                printf("\t%.*s", (int) identity.size(), identity.data());
            }
            else
                printf("\t-"); 
        }
        ...
        for(uint32_t nodeIdx = 0; nodeIdx < tree.nodes.size(); ++nodeIdx)
        {
            ...
            if(node.leaf())
                printf("\t%d", node.leaf_node().encryption_key.data.data()[6]);
            else if(node.blank())
                printf("\t-");
            else
                printf("\t%d", node.parent_node().public_key.data.data()[6]); 
        }
        printf("\n");
    }
