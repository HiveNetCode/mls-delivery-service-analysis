...
* `central-delivery-service/`: contains our implementation of a centralized Delivery Service.
* `centralized-client/`: contains our implement of an MLS client that can be used with the central Delivery Service, but is vulnerable to invalid proposal reference attack.
...
* `attacker/`: contains a Proof-of-concept of an attacker that can emit invalid commits with invalid proposal reference.
* `ds-aware-client/`: implements a client that follows the solution proposed in the article. Thus, this client resists invalid proposal reference attacks and stays in a coherent state with all clients implementing the same solution.
...
Then, all sub-projects can be directly built using the main Makefile and the command `make`.
...
Here we detail, and transcribe the two executions related the paper: the case where an attacker manages to alter the normal function of another client in the group, and the case where client is built following the proposed solution and therefore rejects the commit with an invalid proposal reference.
